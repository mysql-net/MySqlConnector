				...
				})
				{
					if (success)
					{
						...
						using (var reader = cmd.ExecuteReader())
						{
							...
							Assert.Equal(new DateTime(2001, 2, 3, 14, 5, 7, kindIn), reader.GetValue(1));
							Assert.Equal(new DateTime(2001, 2, 3, 14, 5, 6, 800, kindIn), reader.GetValue(2));
							Assert.Equal(new DateTime(2001, 2, 3, 14, 5, 6, 790, kindIn), reader.GetValue(3));
